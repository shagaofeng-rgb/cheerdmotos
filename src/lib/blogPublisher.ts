...
export async function publishDailyAutomatedBlog(target = DAILY_BLOG_TARGET) {
  ...
  for (let index = 0; index < remainingTarget; index += 1) {
    ...
    const post: ContentPost = {
      ...
      imageSourceUrl: 'https://www.cheerdmotos.com',
    ...
    await recordSitemapContentChange({type: 'blog', action: 'published', slug: post.slug, title: post.title});
