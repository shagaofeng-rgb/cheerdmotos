...
async function readPayload(request: Request): Promise<ArticlePayload> {
  ...
  return {
    sign: text(raw.sign, 512),
    classId: text(raw.class_id, 120),
    title: text(raw.title, 220),
    content: text(raw.content, 50_000),
    authorId: text(raw.author_id, 120),
    imageUrl: text(raw.image_url, 1_200)
  };
}
...
export async function POST(request: Request) {
  ...
  await writeAdminStore((store) => {
    ...
    return {
      ...
      posts: [...store.posts, {
        ...
        category: payload.classId || 'blog',
        content,
        publishDate: now.slice(0, 10),
        author: payload.authorId || 'External Blog Plugin',
        source: 'External blog plugin webhook',
        tags: [payload.classId || 'blog', 'external-plugin'],
        seoTitle: `${payload.title} | CHEERDMOTO`,
        seoDescription: excerpt.slice(0, 160),
        sourceName: 'External Blog Plugin',
        sourceUrl: 'https://www.cheerdmotos.com',
        sourceFingerprint: fingerprint,
        contentHash: crypto.createHash('sha256').update(content).digest('hex'),
        imageAlt: `${payload.title} cover image`,
        imageSourceUrl: payload.imageUrl,
        imageCredit: payload.imageUrl ? 'Provided by external blog plugin.' : 'CHEERDMOTO default product image.',
        status: 'published',
        createdAt: now,
        updatedAt: now
      }]
  ...
  await appendAuditLog({actor: payload.authorId || 'external-blog-plugin', action: 'Webhook blog publish', module: 'Blog', result: 'success', ip, userAgent, detail: duplicate ? `Duplicate accepted for ${publishedSlug}.` : `Published ${publishedSlug}.`});
