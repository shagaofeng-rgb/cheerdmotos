...
NEXT_PUBLIC_SITE_URL=https://cheerdmotos.com
```
...
```text
/checkout?product=xceed-electric-dirt-bike&qty=1
```
...
```text
http://localhost:3000/checkout?product=xceed-electric-dirt-bike&qty=1
```
...
curl -X POST http://localhost:3000/api/payments/oceanpayment/simulate-notice \
  ...
  -d "{\"orderId\":\"CM-ORDER-ID\",\"status\":\"paid\"}"
```
